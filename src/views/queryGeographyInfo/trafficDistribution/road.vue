		...
		<div class="boxCenter">
			<div class="search">
				<el-form ref="form" :model="form" label-width="100px" :inline="false">
					<h3>条件查询</h3>
					<el-col :span="12">
						<el-form-item label="道路名称："><el-input v-model="form.roadName" size="mini" style="width: 60%;"></el-input></el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="道路等级：">
							<el-select v-model="form.roadClass" placeholder="请选择" size="mini" style="width: 60%;">
								<el-option v-for="item in form.roadClassList" :key="item.roadClass" :label="item.roadClass" :value="item.roadClass"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
				</el-form>
			...
			<div class="tableBox">
				<h3>查询结果</h3>
					<el-table :data="tableData" v-loading="isLoad" border height="calc(100% - 100px)" style="width: 100%">
						...
						<el-table-column prop="roadName" label="道路名称"  align="center"></el-table-column>
						<el-table-column prop="roadClass" label="道路等级"  align="center"></el-table-column>
						<el-table-column prop="roadCover" label="道路区间"  align="center"></el-table-column>
						<el-table-column prop="roadLength" label="道路长度"  align="center"></el-table-column>
						<el-table-column prop="roadWidth" label="道路宽度"  align="center"></el-table-column>
						<el-table-column prop="roadCapacity" label="最大装载量"  align="center"></el-table-column>
						<el-table-column prop="roadNote" label="简介" width="350" align="center"></el-table-column>
					</el-table>
					<el-pagination
...
export default {
	...
	methods: {
		commonPart() {
			//获取历史强震的数据列表及总记录数
			let params = { roadName: this.form.roadName, roadClass: this.form.roadClass, pageNo: this.pageNo, pageSize: this.pageSize };
			request({ url: '/getRoadCountAndListAndClass', method: 'post', data: params }).then(res => {
				let data = res.data.result;
				console.log(data);
				this.tableData = data.list;
				this.count = data.count;
				this.form.roadClassList = data.selectList;
				this.isLoad = false;
			});
		},
		//重置数据
